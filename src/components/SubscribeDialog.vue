...
<template>
    <div class="fixed top-0 left-0 w-screen h-screen bg-[rgba(0,0,0,0.5)] transition-all duration-500 ease-in flex justify-center items-center"
        :class="props.isOpen ? 'z-999 opacity-100' : '-z-10 opacity-0'" @click="$emit('handleClose')">
        <form class="w-[500px] p-8 bg-white rounded" @click.stop="" @submit.prevent="subscribe">
            ...
            <Button class="w-full !py-[14px] !text-[0.94rem]" type="submit">Subscribe
                <span v-if="store.networkOperation == 'subscribe'"
